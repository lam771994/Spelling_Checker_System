        ...
        for i in range(1, dp.shape[0]):  #column
            for j in range(1, dp.shape[1]): #row
                dp[i, j] = min(
                    ...
                    dp[i, j - 1] + 1, #deletion
                    dp[i - 1, j - 1] + 0 if text[i - 1] == target[j - 1] else dp[i - 1, j - 1] + 2) #substitution
...
path_to_pdf = '/Users/lamyingxian/Dropbox/*MSc Data Science & Business Analytics/2. Natural Language Processing/3. Assignment sheets-20210917/Assignment 2 - Implementation Report/0749453893-Digital-Marketing.pdf'
